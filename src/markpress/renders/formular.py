...
class FormulaRenderer(BaseRenderer):
    ...
    def render_block(self, latex: str, **kwargs):
        ...
        try:
            ...
            # kif = KeepInFrame(max_w, max_h, [img], mode='shrink', hAlign='CENTER', vAlign='TOP')
            # return [kif, Spacer(1, 4 * mm)]
    ...
    def _generate_image(self, latex: str, fontsize: float, dpi: int = 300):
        ...
        # 1. 配置 Matplotlib
        # 'stix' 字体风格最接近标准 LaTeX
        plt.rc('mathtext', fontset='stix')

        # 2. 创建微型画布
        # figsize 设得很小，完全依赖 bbox_inches='tight' 自动撑开
        fig = plt.figure(figsize=(0.01, 0.01))

        # 3. 绘制文字
        # $$ 包裹以启用 TeX 模式
        # 注意：这里可能需要 try-catch 捕获 LaTeX 语法错误
        fig.text(0, 0, f"${latex}$", fontsize=fontsize)

        # 4. 保存到临时文件
        # 使用 tempfile 生成唯一路径，且不自动删除 (ReportLab 读取需要文件存在)
        # 在生产环境中，这些临时文件应该在程序结束时清理，或者定期清理 /tmp
        fd, path = tempfile.mkstemp(suffix=".png")
        os.close(fd)  # 关闭文件描述符，释放给 plt 使用

        # 渲染保存
        # transparent=True 保证背景透明，融合纸张颜色
        # pad_inches=0.02 留极少量的白边，防止切掉积分号等大符号的边缘
        plt.axis('off')
        plt.savefig(path, format='png', bbox_inches='tight', pad_inches=0.02, dpi=dpi, transparent=True)
        plt.close(fig)  # 必须关闭，防止内存泄漏

        # 5. 计算物理尺寸 (Points)
        # Matplotlib 仅仅保存了像素，我们需要将其换算回 PDF 的 Points 单位
